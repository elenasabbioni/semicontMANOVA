...
estimation <- function(x, n, lambda = c(0, 100), lambda0 = c(0, 100), ncpus, B, penalty, ident = TRUE) {
...
simula <- function(dorep, n, p, lambda = c(0, 100), lambda0 = c(0, 100), pmiss, rho, ncpus = 50L, B, penalty, ident, file.name, mc.cores, tySpecific, ty, c1c2) {
...
simula.internal <- function(i, n, p, lambda = c(0, 100), lambda0 = c(0, 100), pmiss, rho, ncpus, B, penalty, ident, file.name, temp=TRUE, tySpecific, c1c2) {
  # set the seed
  set.seed(1235+i)
  ...
  if (temp) {
    file.name <- paste("./temp", tySpecific, "/",i, "-", file.name, sep="")
  }
