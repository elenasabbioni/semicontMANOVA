...
dir.create(paste("./Results_", method, sep = ""), showWarnings = FALSE)
dir.create(paste("./Results_", method, "/K", K, sep = ""), showWarnings = FALSE)
...
for(ty in names(typeC)){
  ...
  if (ndorep==1) {
    simula.internal_real(dorep, n, p, lambda = lambda, lambda0 = lambda0, ncpus = ncpus, B = B, penalty = penalty, ident = ident, file.name = file.name, temp = FALSE, ty = ty,  mu0 = mu0, sigma0 = sigma0, pmiss0 = pmiss0, mu1 = mu1, sigma1 = sigma1, pmiss1 = pmiss1)
  } else if (ndorep > 1) {
    simula_real(dorep, n, p, lambda = lambda, lambda0 = lambda0, ncpus = ncpus, B = B, penalty = penalty, ident = ident, file.name = file.name, mc.cores = mc.cores, ty = ty, mu0 = mu0, sigma0 = sigma0, pmiss0 = pmiss0, mu1 = mu1, sigma1 = sigma1, pmiss1 = pmiss1)
  }
}
