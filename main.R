...
n_sizes <- c(10, 20) # total sample size in the two groups 
alln <- list()
for(n in n_sizes){
  alln[[as.character(n)]] <- c(n/2, n/2) # sample size in each group
} 
      ...
      for (pmiss in allpmiss){
        if( (typeC[[ty]][2] + pmiss) < 1){ # check if we have an admissible probability \pi_j2
          for (rho in allrho){
            ...
            dir.create(paste("./temp",tySpecific, sep = ""), showWarnings = FALSE)
            
            cat(tySpecific, " --- n: ", n, ", p:", p, ", pmiss: ", pmiss, ", rho: ", rho, "\n", sep = " ")
            file.name <- paste(method,"-", ty, "-", 2*n[1], "-", p, "-", pmiss, '-', rho, '.txt', sep='')
            ...
            if(file.exists(paste("./Results/", ty, "/", file.name, sep = ""))){
              ...
              for(i in 1:dim(old)[1]){
                file.name.i <- paste("./temp", tySpecific, "/", i, "-", file.name, sep="")
            ...
            for (irep in dorep) {
              file.irep <- paste("./temp", tySpecific, "/", irep, "-", file.name, sep="")
            ...
            if (ndorep==1) {
              simula.internal(dorep, n, p, lambda = lambda, lambda0 = lambda0, pmiss, rho, ncpus = 50L, B, penalty = penalty, ident = ident, file.name, temp=FALSE, tySpecific = tySpecific, c1c2 = typeC[[ty]])
            } else if (ndorep > 1) {
              simula(dorep, n, p, lambda = lambda, lambda0 = lambda0, pmiss, rho, ncpus = 50L, B, penalty = penalty, ident = ident, file.name, mc.cores, tySpecific = tySpecific, ty = ty, c1c2 = typeC[[ty]])
            }
          }
        }
      }
