...
table1 <- df[, c(which(colnames(df) %in% c("c1", "c2", "pmiss10.50", "rho10.50")), which(grepl("propRej", colnames(df))))]
table1[, c(5:12)] <- round(table1[, c(5:12)], 3)
table1 <- rbind(c(rep("", 4), rep("n = 10", 4), rep("n = 20", 4)), table1)
colnames(table1) <- c("$c_1$", "$c_2$", "$\\pi_{j1}$", "$\\rho$", rep(c("$p = 50$", "$p = 100$", "$p = 150$", "$p = 200$"), 2) )
